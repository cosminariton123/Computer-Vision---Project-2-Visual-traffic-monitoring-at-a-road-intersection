...
def solve_task_1(root_dir, context_videos_dir, output_dir, visualize=False):
    ...
    TASK_1_DIR = os.path.join(root_dir, "Task1")
    ...
    images = [cv2.imread(os.path.join(TASK_1_DIR, filepath)) for filepath in os.listdir(TASK_1_DIR) if ".jpg" in filepath]
    queries_paths = [os.path.join(TASK_1_DIR, filepath) for filepath in os.listdir(TASK_1_DIR) if ".txt" in filepath]
